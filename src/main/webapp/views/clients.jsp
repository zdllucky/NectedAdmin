...
<html lang="en">
<head>
    <title>Active clients Management</title>
...
<%
    ...
    List<Pair<String, String>> countryList = (List<Pair<String, String>>) request.getAttribute("countries_list");
    ...
    long date;
    Map<String, Long> connectionsByCountries = clients.stream().
            collect(Collectors.groupingBy(Client::getCountryFrom, Collectors.counting()));
%>
...
<div class="container">
    ...
    <div class="row mt-4 mx-1">
        <div class="col">
            <h1 class="display-4">Active clients brief</h1>
        </div>
    </div>
    <div class="row mb-4">
        <div class="col">
            <div class="card-group mb-3">
                <div class="card ">
                    ...
                    <div class="card-body">
                        <dl class="row px-3">
                            <dt class="col-5" style=" margin-bottom: 15px;">Clients amount:</dt>
                            <dd class="col-7" style=" margin-bottom: 15px;"><%out.print(total_amount);%></dd>
                            <dt class="col-5"
                                style="border-top: 1px solid #e6e6e6; margin-bottom: 15px; padding-top: 15px">Purchase
                                stats:
                            </dt>
                            <dd class="col-7"
                                style="border-top: 1px solid #e6e6e6; margin-bottom: 15px; padding-top: 15px">Soon!
                            </dd>
                            <dt class="col-5"
                                style="border-top: 1px solid #e6e6e6; margin-bottom: 15px; padding-top: 15px">Average
                                plan duration:
                            </dt>
                            <dd class="col-7"
                                style="border-top: 1px solid #e6e6e6; margin-bottom: 15px; padding-top: 15px">Soon!
                ...
                <div class="card">
                    <div class="card-header bg-warning h5">Connections from</div>
                    <div class="card-body pt-1">
                        <dl class="row px-3">
                            <%
                                for (Map.Entry<String, Long> tCountry : connectionsByCountries.entrySet()) {
                                    out.print("<dt class=\"col-5\" style=\"border-bottom: 1px solid #e6e6e6; margin-top: 15px; padding-bottom: 15px\">" +
                                            countryList.stream().filter(country -> country.getKey().contentEquals(tCountry.getKey())).findFirst().orElse(new Pair<>("--", "None")).getValue() + " (" + tCountry.getKey() + ")</dt>" +
                                            "<dd class=\"col-7 mb-0\" style=\"border-bottom: 1px solid #e6e6e6; margin-top: 15px; padding-bottom: 15px\">" +
                                            tCountry.getValue() + " connections</dd>");
                                }
                            %>
                        </dl>
                    </div>
    ...
    <div class="row mt-4 mx-1">
        <div class="col">
            <h1 class="display-4">Active clients list</h1>
        </div>
        <div class="col">
            ...
            <%
                if (clients.size() > 0) {
                    out.println("<table id=\"table\" class=\"table " + (clients.size() > 100 ? "table-sm " : "") + "table-hover table-responsive-sm table-striped table-intel table-listing\"><thead><tr class=\"bg-primary text-light\">" +
                            ...
                            "<th scope=\"col\" class=\"filter\">Connected to</th>" +
                            "<th scope=\"col\" class=\"filter\">From</th>" +
                            "<th scope=\"col\" class=\"filter\">Paid for</th>" +
                            "</tr></thead><tbody>");

                    for (Client client : clients) {
                        date = (client.getSubscrTo() - System.currentTimeMillis() / 1000L) / 86400L;
                        out.print("<tr scope=\"row\"><td><a href=\"" + getServletConfig().getServletContext().getContextPath() + "/manage_client?id=" + client.getId() + "\">" + client.getId() + "</a></td><td>" +
                                ...
                                "<a href=\"" + request.getContextPath() + "/manage_server?id=" + client.getServer().getId() + "\">" +
                                client.getServer().getId() + "</a> (" + client.getServer().getCountry() + ")</td><td>" +
                                countryList.stream().filter(country -> country.getKey().contentEquals(client.getCountryFrom())).findFirst().orElse(new Pair<>("--", "None")).getValue() + " (" + client.getCountryFrom() + ")</td><td>" +
                                date + " days</td></tr>");
                    }
