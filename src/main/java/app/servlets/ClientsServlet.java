...
public class ClientsServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (req.getSession(false).getAttribute("logon") == null) {
			req.getSession(false).setAttribute("toPage", "clients");
		...
		try {
			...
			req.setAttribute("rows_amount", DbHandler.getInstance().getActiveClientsAmount());
			if (req.getParameter("page") != null) {
				...
				if (page > 0) {
					if (req.getParameter("by") != null) {
						...
						if (by < 1000 && by > 0) {
							req.setAttribute("clients", DbHandler.getInstance().getActiveClientList((page - 1) * by, by));
							req.setAttribute("page_number", page);
							req.setAttribute("by", by);
						} else {
							req.setAttribute("clients", DbHandler.getInstance().getActiveClientList((page - 1) * 100, 100));
							req.setAttribute("page_number", page);
				...
				req.setAttribute("clients", DbHandler.getInstance().getActiveClientList(0, 100));
